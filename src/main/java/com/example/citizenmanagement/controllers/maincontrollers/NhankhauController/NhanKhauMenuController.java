...
public class NhanKhauMenuController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        tim_kiem_text.textProperty().addListener((observableValue, oldval, newval) -> {
            if(newval.isEmpty()){
                ...
                ResultSet resultSet = Model.getInstance().getDatabaseConnectionNhanKhau().timkiem(tim_kiem_text.getText());
    ...
    public void capnhat() {
        ResultSet resultSet = Model.getInstance().getDatabaseConnectionNhanKhau().truyvan();
        list_view.getItems().clear();
        try{
            if(resultSet.isBeforeFirst()){
                while (resultSet.next()) {
